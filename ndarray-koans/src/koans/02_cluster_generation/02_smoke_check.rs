...
mod cluster_generation_smoke_check {
    ...
    /// give it a go!
    fn smoke_check() {
        let a: Array2<f64> = generate_cluster(10, 2);
        let filename = "python/cluster_smoke_check.npy";

        write_npy("array.npy", a).expect("Failed to write array in npy format.");
        let b: Array2<f64> = read_npy("array.npy").expect("Failed to read array from npy format.");
